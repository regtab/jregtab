...
the RTL grammar, then walks the parse tree with two visitors:

| Stage | Class | Role |
|---|---|---|
| Parse | `RTLLexer` / `RTLParser` (generated from `RTL.g4`) | RTL text → parse tree |
| Build ATP | `ATPBuilder` (`RTLBaseVisitor`) | parse tree → `TablePattern` and nested pattern objects |
...
On any lexer or parser error, `compile` throws `RtlCompileException` carrying the line and
...
`RtlCompileException` for those too.

The result is exactly the same kind of `TablePattern` you could build with the Java fluent API
...
**Serialization** (`ru.icc.regtab.rtl.AtpToRtlSerializer.serialize(pattern)`) performs the
