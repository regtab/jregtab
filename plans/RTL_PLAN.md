...
Задача 52 требует инжекции константной пары (YEAR, '2025') в каждую запись без опоры на какую-либо ячейку таблицы. Текущая грамматика не поддерживает литеральные значения как провайдеры.
...
### Дизайн-решение: Вариант 3 с квотированными тегами
...
| Тег (ячейки) | `#'head'` | контентный тег-фильтр |
| Тег (ограничение) | `TAG #'t1' #'t2'` | фильтр по тегам |
| Константный AVP | `@'YEAR'='2025'` | ATTR+VAL-провайдер в REC |

### 1. Квотирование тегов (`#tag` → `#'tag'`)

**Область изменений:** RTL.g4, RtlCompilerTest (2 строки).

Изменение в грамматике — замена лексерного токена `TAG` на парсерное правило `tagItem`:
...
`HASH` — новый односимвольный токен; `STRING` — уже существующий, поддерживает любые кавычки.

**Миграция кода:** в `ATPBuilder` и `ProviderTemplateResolver` везде, где читается `TAG().getText()` — заменить на `tagItem().STRING().getText()` с применением `StringExtractorFactory.parseStringLiteral(...)`.

**Миграция тестов:** только `RtlCompilerTest.java`:
```java
// Было:
...
### 2. Константный AVP-провайдер (`@'ATTR'='VALUE'`)

**Область изменений:** RTL.g4, ATPBuilder, ProviderSpec, WorkingState.
...
`AT` — новый односимвольный токен. `ASSIGN` (`=`) уже существует в грамматике.

#### ProviderSpec

Новый фабричный метод и поле:
...
`ctxAvpSpec` разрешён только в контексте `->REC` и `->CONCAT`; в `->AVP` — ошибка компиляции (аналогично существующей валидации).

#### WorkingState.applyRec

При обходе провайдеров записи: если провайдер — `ctxAvp`, инжектировать константную пару напрямую в рабочее состояние:

```java
for (Item item : items) {
    if (item instanceof ContextDerivedItem cdi && cdi.spec().constValue() != null) {
        // Константный AVP: name = cdi.str(), value = cdi.spec().constValue()
...
Метод `injectConstAvp` добавляет запись в `avp`-маппинг без якорной ячейки — пара появится в каждой записи, использующей данный REC-якорь.

#### Порядок атрибутов в схеме

Константные AVP-провайдеры вносятся в схему в порядке их появления в `provSpecs`. Для задачи 52 порядок: ND, AIRLINE, AIRPORT, MON, YEAR — YEAR последний, т.к. `@'YEAR'='2025'` стоит четвёртым в `(^SC, -LT, CL, @'YEAR'='2025')->REC`.

### Итоговая последовательность реализации

1. Квотирование тегов: изменить RTL.g4 (TAG → tagItem), обновить ATPBuilder/ProviderTemplateResolver, исправить 2 строки в RtlCompilerTest — запустить все тесты.
2. Токен `@` и правило `ctxAvpSpec`: добавить в RTL.g4.
3. `ProviderSpec.ctxAvp`: добавить поле `constValue` в `ContextLiteralSpec`, новый фабричный метод.
4. `ATPBuilder.buildProvSpec`: ветка для `ctxAvpSpec`.
5. `WorkingState.injectConstAvp` + интеграция в `applyRec`.
6. `RtlTask52Test` + проверка всех 5 вариантов.
7. Обновить `AtpToRtlSerializer` для сериализации `ctxAvp`-провайдеров.
